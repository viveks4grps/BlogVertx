...
(function($) {
	...
	app.controller('AppHomeController', function($http, $log, $scope,
		...
		$http.get('/Services/rest/loadblogs').success(
		...
		$http.get('/Services/rest/user?signedIn=true').success(
				function(data, status, headers, config) {
					...
					ws = $websocket.$new('ws://'+$location.host()+':'+$location.port()); // instance of ngWebsocket, handled by $websocket service
			        ...
			        ws.$on('$message', function(data){
			        	 ...
			        	 if(data.event==="UserLogin"){
			        		 ...
			        		 $log.debug(data.data.messageObject);
			        		 if($scope.chatMessages===undefined)
			        			 $scope.chatMessages=[];
			        		 $scope.chatMessages.push(data.data.messageObject);
			...
			$scope.tagSearch = function(){
				$http.get('/Services/rest/loadblogs?tag='+$scope.searchTag).success(
					function(data, status, headers, config) {
						$scope.blogs = data;
						$scope.loading = false;
					}).error(function(data, status, headers, config) {
			...
			$scope.sendMessage = function(chatMessage){
				...
				ws.$emit('chatMessage', {'messageObject':{'sender':$rootScope.userName,'text':chatMessage}
	...
	//------------------------------------------------------------------------------------------------------------------
	// Controller for the user home page with user blogs 
	//------------------------------------------------------------------------------------------------------------------
	app.controller('MyHomeController', function($http, $log, $scope,
			$rootScope, $websocket, $location) {
		var controller = this;
		$log.debug("MypHomeController...");
		$http.get('/Services/rest/loadMyBlogs').success(
				function(data, status, headers, config) {
					$scope.blogs = data;
					$scope.loading = false;
				}).error(function(data, status, headers, config) {
	...
	app.controller('LoginController', function($http, $log, $scope, $location,
		...
		$scope.submitRegister = function(user){
			...
			$http.post("/Services/users", user).success(
					function(data) {
	...
	app.controller('NavbarController',
			function($http, $log, $scope, $rootScope, $location) {
				...
				$log.debug("Logged in value is..."+$rootScope.loggedIn);
				$scope.logout = function(){
					$log.debug("Logging out...");
					$http.post("/Services/rest/logout/").success(
							function(data) {
