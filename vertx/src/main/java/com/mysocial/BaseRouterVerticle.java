...
public class BaseRouterVerticle extends AbstractVerticle {

	//private String currentNodeId = "Node1";
	//private int currentNodePort = 9090;
	
	protected String currentNodeId = "Node1";
	protected int currentNodePort = 9090;
	...
	public BaseRouterVerticle() {
		
	}
	
	public static void createAndDeployVerticle(String classname,boolean createDistMap,String nodeId) {
		...
		Vertx.clusteredVertx(opts, res -> {
			if (res.succeeded()) {
				...
				vertx.eventBus().consumer(nodeId, m->{
					//System.out.println("Cluster Message meant for Node1: "+m.body());
					CommentsUtil.handleCommentsEventBusMessage((String) m.body());
				});
				CommentsUtil.initCommentsMap(((HazelcastClusterManager)cm).getHazelcastInstance(),createDistMap);
			} else {
